...
def minibatch_parse(sentences, model, batch_size):
	...
	while unfinished_parses:
		...
		minibatch_parses = unfinished_parses[:batch_size] if len(unfinished_parses) > batch_size else unfinished_parses[:]
		...
		while minibatch_parses:
			...
			for i in range(len(minibatch_parses)):
				...
				if (not minibatch_parses[i].buffer) and len(minibatch_parses[i].stack) == 1:
					minibatch_dependencies[minibatch_index[i]] = minibatch_parses[i].dependencies
